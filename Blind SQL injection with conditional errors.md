...
<br><br> Firstly, to verify that a user named administrator is present we must change one of the conditions in the payload. Therefore, I tried using this payload:
...
<br><br> To find the length of the password, I tried using this payload ```' ||(SELECT CASE WHEN (1=2) THEN TO_CHAR(1/0) ELSE NULL END FROM users WHERE username = 'administrator' AND length(password) > 2)||'``` however an error always occured, no matter if the length was > 2, < 2 or = 2.
...
This error exist because the developer must have assumed that the cookie is harmless, meaning that they don't take proper measures to secure it. As a result, we are able to attach our own queries to the cookies, causing the website to perform functions which it wasn't designed to. As a result, we can check if a certain condition is true, by seeing if the server responds with an error.
